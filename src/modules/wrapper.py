...
import numpy as np
from gymnasium.wrappers import AtariPreprocessing, FrameStackObservation, RecordVideo, TimeLimit
...
def get_atari_env(
    env_id: str, render_mode: str = "rgb_array", record_folder: Path | None = None, record_frequency: int = 50
) -> gym.Env:
    ...
    if record_folder is not None:
        env = RecordVideo(env=env, video_folder=record_folder, episode_trigger=lambda x: x % record_frequency == 0)
    return TimeLimit(
